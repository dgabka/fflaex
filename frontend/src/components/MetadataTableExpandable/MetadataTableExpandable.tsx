...
export default function MetadataTableExpandable({ tags }: Props): JSX.Element {
  ...
  const handleExpandMetadata = () => {
    setIsMetadataExpanded(!isMetadataExpanded);
  };
  return (
    <div
      className={`${classes["meta-container"]} ${
        isMetadataExpanded ? classes.expanded : ""
      }`}
    >
      <button
        onClick={handleExpandMetadata}
        className={classes["meta-expand-btn"]}
      >
        Metadata{" "}
        <svg
